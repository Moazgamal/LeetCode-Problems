class Solution {   
    public:
        int daysBetweenDates(string date1, string date2) {
            ...
            return abs(date_to_int(date2,minyear) - date_to_int(date1,minyear));
        }
        
    protected:
        int date_to_int(string s_date, int minyear)
        {
            ...
            int date = 0;
            for(int y = minyear; y < Y; ++y) date += is_leap_year(y) ? 366 : 365;
